...
const ReviewsTable = ({ data = [], onReply }) => {
  ...
  const allReviews = React.useMemo(() => (Array.isArray(reviews) ? reviews : []), [reviews]);
  const unansweredCount = React.useMemo(() => allReviews.filter((item) => !item.review?.response).length, [allReviews]);
  const answeredCount = React.useMemo(() => allReviews.filter((item) => item.review?.response).length, [allReviews]);
  ...
  const saveReply = (reviewData) => {
    ...
    dispatch(postReviewResponse({ reviewId, response: responseText }))
      .then(() => {
        ...
        setTab(2);
        const status = tab === 0 ? 'все' : tab === 1 ? 'неотвеченные' : 'отвеченные';
        ...
        <Tab
          label={
            <TabLabelContainer>
              Неотвеченные
              {unansweredCount > 0 && (
                <UnansweredCount component="span">+{unansweredCount}</UnansweredCount>
              )}
            </TabLabelContainer>
          }
          sx={{
          ...
          <TableBody>
            {currentReviews.length > 0 ? (
              currentReviews.map((item, index) => {
                ...
                const commentWords = item.review?.comment?.split(' ') || [];
                const reviewDate = new Date(item.review?.createdAt);
                const formattedDate = `${reviewDate.toLocaleDateString('ru-RU')} - ${reviewDate.toLocaleTimeString('ru-RU', {
                  hour: '2-digit',
                  minute: '2-digit',
                })}`;
                ...
                return (
                  <React.Fragment key={id}>
                    <StyledTableRow>
                      ...
                      <TableCell>
                        <ProductAvatar
                          src={item.product?.image || '/default-product-image.jpg'} // Туура: product.image - бир URL
                          alt={item.product?.name}
                          onClick={() => handleImageClick(item.product?.image)}
                        />
                      </TableCell>
                      <TableCell>
                        <ProductName variant="body2">{item.product?.name}</ProductName>
                        <ProductModel variant="caption">{item.product?.brand}</ProductModel>
                        <br />
                      ...
                      <TableCell>
                        <ExpandableComment>
                          <Typography variant="body2">
                            {isExpanded
                              ? item.review?.comment
                              : truncateText(item.review?.comment)}
                          </Typography>
                          {commentWords.length > 5 && (
                            ...
                            >
                              {isExpanded && commentWords.slice(5).join(' ')}
                            </Typography>
                          )}
                          {isExpanded && item.review?.images?.length > 0 && (
                            <ReviewImages>
                              {item.review.images.slice(0, 5).map((img, idx) => (
                                <ReviewImage
                                  key={idx}
                                  src={img || '/default-review-image.jpg'} // Туура: images массивинен алынат
                          ...
                          <ReviewDate variant="caption" display="block" sx={{ mt: 1 }}>
                            {formattedDate}
                          </ReviewDate>
                        </ExpandableComment>
                      </TableCell>
                      ...
                      <TableCell>
                        <UserInfoContainer>
                          <ClickableAvatar
                            src={item.user?.profile || '/default-user-profile.jpg'} // Туура: user.profile - бир URL
                            alt={item.user?.fullName}
                            onClick={() => handleImageClick(item.user?.profile)}
                            $hasProfile={!!item.user?.profile}
                          />
                          <UserDetails>
                            <UserName variant="body2">{item.user?.fullName}</UserName>
                            <UserEmail variant="caption">{item.user?.email}</UserEmail>
                          </UserDetails>
                        </UserInfoContainer>
                      </TableCell>
                      <TableCell align="right">
                        <ActionsContainer>
                          <IconButton size="small" onClick={() => handleDeleteClick(id)}>
                            <StyledDeleteIcon src={Icons.deleteb} alt="" />
                          </IconButton>
                          <IconButton size="small" onClick={() => toggleExpanded(id)}>
                            {isExpanded ? (
                              <img src={Icons.arrowDown} alt="" sx={{ fontSize: 28, color: '#999' }} />
                            ) : (
                              <img src={Icons.arrowUp} alt="" sx={{ fontSize: 28, color: '#999' }} />
                            )}
                          </IconButton>
                        </ActionsContainer>
                      </TableCell>
                            ...
                            <ExpandedContent>
                              <ReplyContainer>
                                <ReplySection>
                                  ...
                                  {item.review?.response && !editingReviews[item.review?.id] ? (
                                    <>
                                      <ResponseContainer>
                                      ...
                                      <ActionButton
                                        variant="edit"
                                        onClick={() => startEditing(item.review?.id, item.review?.response)}
                                      >
                                        Редактировать
                                      </ActionButton>
                                    </>
                                  ) : editingReviews[item.review?.id] ? (
                                    <>
                                      <StyledTextField
                                        fullWidth
                                        multiline
                                        ...
                                        value={replyTexts[item.review?.id] || ''}
                                        onChange={(e) =>
                                          setReplyTexts((prev) => ({
                                            ...prev,
                                            [item.review?.id]: e.target.value,
                                          }))
                                        }
                                        variant="outlined"
                                      ...
                                      <ButtonContainer>
                                        <CancelButton
                                          variant="outlined"
                                          size="medium"
                                          onClick={() => cancelEditing(item.review?.id)}
                                        >
                                          Отменить
                                        </CancelButton>
                                        <SaveButton
                                    ...
                                    <ActionButton
                                      variant="reply"
                                      onClick={() => startEditing(item.review?.id)}
                                    >
                                      Ответить
                                    </ActionButton>
                                  )}
              ...
              <TableRow>
                <NoReviewsCell colSpan={7}>
                  <Typography variant="body1">Пока отзывы не найдены</Typography>
                </NoReviewsCell>
              </TableRow>
            )}
          </TableBody>
        ...
        <Modal open={modalOpen} onClose={handleCloseModal}>
          <ModalContent>
            ...
            <ModalButtonContainer>
              ...
              <ModalButton variant="contained" onClick={handleConfirmDelete}>
                Да
              </ModalButton>
            </ModalButtonContainer>
          </ModalContent>
...
const UnansweredCount = styled(Typography)({ color: '#03ff0b', fontWeight: 'bold' });
...
const UserInfoContainer = styled(Box)({ display: 'flex', alignItems: 'center', gap: '8px' });
...
const StyledDivider = styled(Divider)({ borderColor: '#E0E0E0', padding: '0px', margin: '0px' });
const DividerCell = styled(TableCell)({ padding: '0px', borderBottom: 'none' });
const NoReviewsCell = styled(TableCell)({ textAlign: 'center', padding: '20px' });
