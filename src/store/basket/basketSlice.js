...
const basketSlice = createSlice({
   ...
   reducers: {
      ...
      toggleSelect: (state, action) => {
         ...
         const item = state.basketItems.find(
            (i) => i.productTypeId === productTypeId
         )
   ...
   extraReducers: (builder) => {
      builder
         ...
         .addCase(fetchBasket.rejected, (state, { payload }) => {
            state.loading = false
            state.error = payload
         })
         .addCase(removeFromBasket.fulfilled, (state, { payload }) => {
            state.basketItems = state.basketItems.filter(
               (i) => i.productTypeId !== payload
            )
         })
         .addCase(toggleFavorite.fulfilled, (state, { payload }) => {
            const item = state.basketItems.find(
               (i) => i.productTypeId === payload.productTypeId
            )
...
export const { toggleSelectAll, toggleSelect, increase, decrease } =
   basketSlice.actions
